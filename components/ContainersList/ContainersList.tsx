...
export const ContainersList = ({ containers, setContainers }: ContainersListProps): JSX.Element => {
	return (
		<div className={styles.containersDiv}>
			{containers.map(c => (
				<ContainersItem key={c.value} value={c.value} label={c.label} setContainers={setContainers} />
			))}
		</div>
	);
};
